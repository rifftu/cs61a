...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        def bind_values(formallist, vallist, env):
            if formallist == nil:
                if vallist == nil:
                    return None
                else:
                ...
                raise SchemeError('not enough vals')
            else:
                env.define(formallist.first, vallist.first)
                return bind_values(formallist.second, vallist.second, env)
        bind_values(formals, vals, child_frame)
        return child_frame
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        rest = do_lambda_form(Pair(formals, body), env)
        env.define(target.first, rest)
        return target.first
        # END PROBLEM 10
...
def do_and_form(expressions, env):
    ...
    while expressions.second is not nil:
        firstval = scheme_eval(expressions.first, env)
        if scheme_falsep(firstval):
            return firstval
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        list_formals = Pair(bindings.first.first, list_formals)
        list_vals = Pair(scheme_eval(bindings.first.second.first, env), list_vals)
        bindings = bindings.second
    check_formals(list_formals)
    return env.make_child_frame(list_formals, list_vals)
    # END PROBLEM 15
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        while (isinstance(result, Thunk)):
            ...
            if scheme_symbolp(first) and first in SPECIAL_FORMS:
                ...
                operands = rest.map(lambda x: optimized_eval(x, env))           
                check_procedure(operator)     
