...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        # BEGIN EC
        "*** YOUR CODE HERE ***"
        if self.is_scared and self.has_ptsd == False:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        def find_bee(newplace, dist):
            if newplace.bees and self.min_range <= dist <= self.max_range:
                return newplace.bees
            if newplace.entrance != hive:
                return find_bee(newplace.entrance, dist + 1)

        return random_or_none(find_bee(self.place, 0))
        # END Problem 3 and 4
...
# The WallAnt class
class WallAnt(Ant):
    """Wallant just sits around and gets hurt"""
...
# END Problem 13
    ...
    def buffAll(self, newplace):
        if newplace.ant:
            self.buff(newplace.ant)
        if newplace.exit:
            self.buffAll(newplace.exit)
...
def apply_effect(effect, bee, duration):
    ...
    oldAction = bee.action
    newDuration = duration
    def countAction(colony):
        nonlocal newDuration
        if newDuration:
            newDuration -= 1
            effect(oldAction, bee)(colony)
        else:
            if effect == make_scare:
                bee.has_ptsd = True
            oldAction(colony)

    bee.action = countAction
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, hive):
        # BEGIN Problem OPTIONAL
        insect_dict = {}
        def collect_targets(newplace, dist, laser_self):
            if newplace.ant and newplace.ant is not laser_self:
                insect_dict[newplace.ant] = dist
            for b in newplace.bees:
                insect_dict[b] = dist
            if newplace.entrance != hive:
                collect_targets(newplace.entrance, dist + 1, laser_self)
        collect_targets(self.place, 0, self)
        return insect_dict
        # END Problem OPTIONAL
