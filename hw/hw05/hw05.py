...
def make_fib():
    ...
    """
    "*** YOUR CODE HERE ***"
    last, curr, index = 1,0,0
    def fib():
        nonlocal last, curr, index
        if index == 0:
            index += 1
            return 0
        else:
...
def make_withdraw(balance, password):
    ...
    "*** YOUR CODE HERE ***"
    faillist = []
    def withdraw(amount, pguess):
        nonlocal balance, faillist
        if len(faillist) < 3:
            if pguess == password:
                if balance >= amount:
            ...
            return "Your account is locked. Attempts: " + str(faillist)
    return withdraw
...
def make_joint(withdraw, old_password, new_password):
    ...
    if type(message) == str:
        return message
    def jointwithdraw(amount, passguess):
        if passguess == new_password:
            return withdraw(amount, old_password)
        else:
            return withdraw(amount, passguess)
...
def generate_paths(t, x):
    ...
    pathslist = []
    goodlist = []
    def pathfinder(t,x,currpath):
        currpath.append(label(t))
        if label(t) == x:
            currpath.append('yeet')
        if not is_leaf(t):
            for b in branches(t):
                newpath = currpath[:]
                pathslist.append(newpath)
                pathfinder(b,x,newpath)
    pathfinder(t,x,[])
    for b in pathslist:
        if b.pop() == 'yeet':
            while 'yeet' in b:
